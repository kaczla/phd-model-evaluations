...
python -m phd_model_evaluations.cli.train.finetuning.run_finetuning_seq2seq \
  --model_name t5-small \
  --train_file glue-lm-gap/train/dataset_data.json.xz \
  --validation_file glue-lm-gap/dev-0/dataset_data.json.xz \
  --dataset_name cola \
  --output_dir out/t5-small-cola
"""

import logging
from typing import List, Optional, Tuple

from transformers import AutoConfig, HfArgumentParser, PretrainedConfig, Seq2SeqTrainer, Seq2SeqTrainingArguments
...
from phd_model_evaluations.train.finetuning.seq2seq_utils import (
    get_prefix_and_key_text,
    get_seq2seq_metrics_fn,
    get_sequence_length_and_padding_strategy_seq2seq,
    get_task_specific_parameters,
    preprocess_seq2seq_data,
    set_task_specific_parameters_in_configuration,
    update_generation_max_length,
)
...
def parse_args(
...
) -> Tuple[ModelArguments, DataTrainingArguments, LoggerArguments, Seq2SeqTrainingArguments]:
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, LoggerArguments, Seq2SeqTrainingArguments),
        description="Run fine-tuning seq2seq model on given data - it's support classification and regression.",
    )
    ...
    if training_args.generation_max_length is None:
        generation_max_length = model_args.target_length // 2
        assert generation_max_length > 1, "Max length for generation cannot be empty!"
        training_args.generation_max_length = generation_max_length

    if training_args.generation_num_beams is None:
        training_args.generation_num_beams = 3

    return model_args, dataset_args, logger_args, training_args


def run_fine_tuning_seq2seq() -> None:
    model_args, dataset_args, logger_args, training_args = parse_args()
    set_logging_from_logger_arguments(logger_args)
    LOGGER.info(f"Using half target length to generation max length: {training_args.generation_max_length}")
    LOGGER.info(f"Using default settings for beam search: {training_args.generation_num_beams}")
    ...
    metric_for_best_model = MetricForBestModel(
        name=training_args.metric_for_best_model, greater_is_better=training_args.greater_is_better
    )
    ...
    id_to_label = {label_to_id[str(label)]: str(label) for label in dataset_configuration.target_label_values}
    prefix_and_key_text = get_prefix_and_key_text(
        dataset_configuration.dataset_name, dataset_configuration.configuration_name
    )
    ...
    train_dataset = preprocess_seq2seq_data(
        train_dataset,
        tokenizer,
        sequence_length,
    ...
    validation_dataset = preprocess_seq2seq_data(
        validation_dataset,
        tokenizer,
        sequence_length,
        model_args.target_length,
    ...
    update_generation_max_length(model_args, training_args, train_dataset, validation_dataset)
    task_specific_parameters = get_task_specific_parameters(
        model_args, training_args, dataset_configuration, label_to_id, id_to_label, prefix_and_key_text
    )
    set_task_specific_parameters_in_configuration(
        model.config, dataset_configuration.get_name(), task_specific_parameters
    )
    compute_metrics_fn = get_seq2seq_metrics_fn(
        dataset_configuration.metric_configuration, tokenizer, label_to_id, is_regression, metric_for_best_model
    )
    ...
    LOGGER.info("Fine-tuning seq2seq finished")


if __name__ == "__main__":
    run_fine_tuning_seq2seq()
