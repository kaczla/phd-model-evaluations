...
from phd_model_evaluations.evaluation.lm_gap.seq2seq_lm_gap_utils import (
    get_special_token_indexes,
    prepare_seq2seq_data_loader,
    prepare_seq2seq_dataloader_for_next_prediction,
)
...
def process_next_predictions(
    ...
    dataloader = prepare_seq2seq_dataloader_for_next_prediction(
        model, tokenizer, input_with_seq2seq_predicted_tokens, batch_size
    )

    predicted_tokens = []
    for batch in dataloader:
        index_predicted_tokens = batch.pop("index_predicted_tokens").tolist()
        outputs: Seq2SeqLMOutput = model(**batch.to(model.device))
        ...
        for index, (token_index_list, probability_list) in zip(
            ...
            previous_predicted_token = input_with_seq2seq_predicted_tokens.predicted_tokens[index]
    ...
    if current_depth < max_depth:
        ...
        for i in range(len(input_with_seq2seq_predicted_tokens.predicted_tokens)):
            input_with_seq2seq_predicted_tokens.predicted_tokens[i].total_score *= 0.5
            input_with_seq2seq_predicted_tokens.predicted_tokens[i].scores.append(0.5)
        ...
        predicted_tokens.extend(
            process_next_predictions(
                model,
                tokenizer,
                InputWithSeq2SeqPredictionTokens(
                    input_indexes=input_with_seq2seq_predicted_tokens.input_indexes,
                    decoder_input_indexes=input_with_seq2seq_predicted_tokens.decoder_input_indexes,
                    prediction_token_index=input_with_seq2seq_predicted_tokens.prediction_token_index,
...
def run_seq2seq_simple(
    ...
    tokenizer_special_indexes = get_special_token_indexes(tokenizer)
    data_loader = prepare_seq2seq_data_loader(tokenized_text, model, tokenizer, predict_token_index)
    ...
    for batch in tqdm(data_loader, total=len(data_loader), desc="Predicting batch of tokens"):
        ...
        next_tokens_scores = torch.softmax(outputs.logits[:, -1, :], dim=-1)
        top_k_tokens = torch.topk(next_tokens_scores, top_k, largest=True)
        for prefix_input_ids, decoder_token_index_list, (token_index_list, probability_list) in zip(
            batch["input_ids"].tolist(),
            batch["decoder_input_ids"].tolist(),
            zip(top_k_tokens.indices.tolist(), top_k_tokens.values.tolist(), strict=True),
            strict=True,
        ):
            # Get predictions
            ...
            input_with_seq2seq_predicted_tokens = InputWithSeq2SeqPredictionTokens(
                input_indexes=prefix_input_ids,
                decoder_input_indexes=decoder_token_index_list,
                prediction_token_index=predict_token_index,
                predicted_tokens=predicted_tokens,
            )
            ...
            if method_arguments.depth > 1:
                ...
                predicted_tokens.extend(
                    process_next_predictions(
                        model,
                        tokenizer,
                        input_with_seq2seq_predicted_tokens,
                        batch_size,
                        1,
